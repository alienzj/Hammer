...
struct compltNum2NumFunctor
{
    bool operator()(long x, long y) const
    {
        ...
        for (int i = 0; i < kmersize; i++)
        {
            ...
            { //! detx
                if (genome.at(x + i) ==
                    revcomp(genome.at(x + kmersize - i - 1)))
                {
                    xchar = genome.at(x + i);
                }
                else
            ...
            { //! dety
                if (genome.at(y + i) ==
                    revcomp(genome.at(y + kmersize - i - 1)))
                {
                    ychar = genome.at(y + i);
                }
                else
...
void usage(int argc, char *argv[]) { exit(1); }
...
int main(int argc, char *argv[])
{
    ...
    goodKmers.resize(unique(goodKmers.begin(), goodKmers.end()) -
                     goodKmers.begin());
    cout << "Number of distinct genomic kmers in the data: " << goodKmers.size()
         << endl;
    ...
    */
    ...
    badKmers.resize(unique(badKmers.begin(), badKmers.end(), compeq) -
                    badKmers.begin());

    /*sort(badKmers.begin(), badKmers.end());
      badKmers.resize(unique(badKmers.begin(), badKmers.end()) -
      badKmers.begin());
     */
    cout << "Number of distinct kmers in the data: "
         << goodKmers.size() + badKmers.size() << endl;
    double spec =
        double(goodKmers.size()) / double(goodKmers.size() + badKmers.size());
    cout << "PPV: " << spec << endl;
    cout << "Sorting genomicKmers (" << genomicKmers.size()
         << " elements)...\n";
    sort(genomicKmers.begin(), genomicKmers.end());
    genomicKmers.resize(unique(genomicKmers.begin(), genomicKmers.end()) -
                        genomicKmers.begin());
    cout << "Number of distinct kmers in the genome: " << genomicKmers.size()
         << endl;
    double sens = double(goodKmers.size()) / double(genomicKmers.size());
    cout << "Sensitivity: " << sens << endl;
    cout << "RES\t" << goodKmers.size() + badKmers.size() << "\t"
         << goodKmers.size() << "\t" << spec << "\t" << sens << endl;
}
