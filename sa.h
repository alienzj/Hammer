...
class suffixArray
{
  public:
    ...
    /*void build(int kmersize, char skip) {
            cerr << "Building suffix array...\n";
            for (int i = 0; i < genSize; i++) {
                    if (genome.find_first_of('$', i, kmersize) != string::npos)
    continue; sa.push_back(i);
            }
    ...
    */
    ...
    void save(string filename, int k = 0)
    {
        ...
        {
            for (int i = 0; i < sa.size(); i++)
                out << sa[i] << "\t" << genome.substr(sa[i], k) << endl;
        }
        out.close();
    }
    ...
    int find(string kmer)
    {
        ...
        vector<int>::iterator it =
            lower_bound(sa.begin(), sa.end(), kmer, complt);
