...
class unionFindClass
{
  public:
    ...
    void unionn(const int &x, const int &y) { link(find_set(x), find_set(y)); }
    ...
    void get_classes(vector<vector<int>> &otherWay)
    {
        ...
        otherWay.erase(remove_if(otherWay.begin(), otherWay.end(), pred1),
                       otherWay.end());
    }

    void get_classes(vector<int> &consolidatedData,
