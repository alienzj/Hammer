...
template <class T> ostream &operator<<(ostream &out, const vector<vector<T>> &v)
{
    out << v.size() << endl;
    if (v.size() != 0)
    {
...
template <class T> int argmax(const vector<T> &data)
{
    if (data.size() == 0)
        return -1;
    T maxVal;
...
int nscore(vector<int> &data, int genLen, int nval)
{
    ...
    {
        totLen += data[i];
        if (totLen >= genLen * double(nval / 100.0))
            return data[i];
    }
...
bool get_row(istream &inFile, vector<string> &row, char delim = '\t')
{ // read a tab delimited line from file
    string line, s;
    row.clear();
    getline(inFile, line);
    if (inFile.eof())
...
bool get_row_whitespace(istream &inFile, vector<string> &row)
{ // read a tab delimited line from file
    string line, s;
    row.clear();
    getline(inFile, line);
...
int hamdist(const string &x, const string &y, HAMDIST_OPTS mode,
            int maxAllowed = -1)
{
    if (mode == ANY_STRAND)
    {
...
template <class T> string make_string(const T &s)
{
    ostringstream o;
    o << s;
    return o.str();
