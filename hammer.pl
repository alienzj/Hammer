...
GetOptions(
    ...
    'threads=i'   => \$threads,
    'cutoff=f'    => \$freqCutoff,
    'trimstart=i' => \$startTrimVal,
    'trimend=i'   => \$endTrimVal,
    'qv_off=i'    => \$qvoffset,
    'k=i'         => \$kmerval,
    'mem=f'       => \$mem,
    'ref=s'       => \$refgen,
...
open( LOGFILE, ">>log.txt" )
  or die("$0 : failed to open log file for output: $!\n");
...
open( READMASTERFILE, "<$readsMaster" )
  or die("$0 : failed to open file $readsMaster: $!\n");
my $readFiles;
...
$command =
"cat $readFiles | $hamdir/preproc $startTrimVal $endTrimVal $qvoffset raw | grep -v 'N' > $preprocReads";
execCommand($command);
...
chomp $filesize;
$command =
"cat $preprocReads | $hamdir/getmers $kmerval $filesize $meminfull | grep -v 'N' > $kmerfile";
execCommand($command);
...
open( CMDFILE, ">cmdSort.txt" )
  or die("$0 : failed to open cmdSort.txt for output: $!\n");
for ( $i = 0 ; $i < $tau + 1 ; $i++ ) {
    $command =
        "cat $kmerfile | $hamdir/splinter $i "
      . ( $tau + 1 )
      . " | LC_ALL=C  sort $sortOpt -k1,1 -T . > $base.$i";
...
chomp $threads2;
$command =
  "$hamdir/cluster $tau $kmerval $kmerfile $readBase $meminfull 0 $threads2 ";
execCommand($command);
...
$command =
" cat $ufcorfile.? |  grep -v '^\$' |  item 3 | grep -v rem | uniq  >reads.fixed.kmers";
execCommand($command);
...
$command =
  "cat $base.fixed.kmers| checkss  -k $valk -g $refgen > scores$valk & ";
execCommand($command);

open( CMDFILE, ">cmdMap.txt" )
  or die("$0 : failed to open cmdMap.txt for output: $!\n");
for ( $i = 0 ; $i < $threads ; $i++ ) {
    $command =
"cat $ufcorfile.$i | grep -v '^\$'  |  dbt map -g $refgen -l 2 | dbt map -g $refgen -l 3 > mapped.$i ";
...
execCommand(
"cat  mapped.* | grep ingle | item 4  10 | awk '{ if (\$1 >= 10) \$1 = 10; print \$1, \$2 }' | sort -k2,2 -k1n,1 | uniq -c > distribmult &"
);
execCommand(
"cat  mapped.* | grep ingle | item 5  10 | awk '{ if (\$1 >= 2) \$1 = 2; print int(\$1*10), \$2 }' | sort -k2,2 -k1n,1 | uniq -c > distribfreq &"
);

MISC:
open( CMDFILE, ">cmdAnalyze.txt" )
  or die("$0 : failed to open cmdAnalyze.txt for output: $!\n");
for ( $i = 0 ; $i < $threads ; $i++ ) {
    $command =
"cat mapped.$i | grep -v mbig | grep -v ingle | awk '{ print \$1, \$10, \$11 }' | countCenters > centerCounts.$i ";
    print CMDFILE "$command & \n";
}
print CMDFILE "wait\n";
close(CMDFILE);
...
execCommand(
"cat centerCounts.* | awk '{ if (\$1 == 0) print \$2 }' | $hamdir/sumit | awk '{ print \"0\\t\" \$1 }' >> centerCounts"
);
execCommand(
"cat centerCounts.* | awk '{ if (\$1 == 1) print \$2 }' | $hamdir/sumit | awk '{ print \"1\\t\" \$1 }' >> centerCounts"
);
execCommand(
"cat centerCounts.* | grep -v mbig | grep -v ingle | awk '{ if (\$1 == 2) print \$2 }' | $hamdir/sumit | awk '{ print \"2\\t\" \$1 }' >> centerCounts"
);
execCommand(
"cat centerCounts.* | grep -v mbig | grep -v ingle | awk '{ if (\$1 == 3) print \$2 }' | $hamdir/sumit | awk '{ print \"3\\t\" \$1 }' >> centerCounts"
);
execCommand(
"cat centerCounts.* | grep -v mbig | grep -v ingle | awk '{ if (\$1 == 4) print \$2 }' | $hamdir/sumit | awk '{ print \"4\\t\" \$1 }' >> centerCounts"
);
