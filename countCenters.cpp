...
void process_block(deque<Line> &block)
{
    ...
    // for (int i = 0; i < block.size(); i++) cout << "\t" << block[i].idx <<
    // "\t"
    // << block[i].h1 << "\t" << block[i].h2 << endl; cout << "CAT\t" <<type <<
    // endl <<  endl;
...
//for (int i = 0; i < block.size(); i++) cout << block[i].count << "\t" <<
block[i].idx << "\t" << block[i].h1 << "\t" << block[i].h2 << endl; cout <<
"CAT\t" <<type << endl <<  endl;
